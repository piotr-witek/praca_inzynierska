...
def generate_expired_inventory_xls(request):
    response = HttpResponse(content_type='application/vnd.ms-excel')
    response['Content-Disposition'] = 'attachment; filename="towary_przeterminowane.xls"'
...
def generate_expired_inventory_csv(request):
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="towary_przeterminowane.csv"'
...
def generate_expiring_inventory_xls(request):
    response = HttpResponse(content_type='application/vnd.ms-excel')
    response['Content-Disposition'] = 'attachment; filename="towary_bliskie_przeterminowaniu.xls"'
...
def generate_low_stock_inventory_xls(request):
    response = HttpResponse(content_type='application/vnd.ms-excel')
    response['Content-Disposition'] = 'attachment; filename="towary_z_niskim_stanem.xls"'
...
def generate_low_stock_inventory_csv(request):
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="towary_z_niskim_stanem.csv"'
...
def generate_suppliers_csv(request, date_from=None, date_to=None):
    ...
    writer = csv.writer(response)
    writer.writerow(["Id", "Nazwa", "Email", "Telefon", "Adres", "Data modyfikacji", "Data utworzenia"])
